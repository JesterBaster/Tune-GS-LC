import { resourceValues } from "./resources.mjs";
...
export class SimpleTokenDocument extends TokenDocument {
  ...
  getBarAttribute(barName, {alternative}={}) {
    ...
    if (/^resources\.(health|mp|rp|ap)$/.test(attr)) {
      const resource = resourceValues(current);
      data.value = resource.value;
      data.max = resource.effectiveMax;
    }
    data.editable = true;
    return data;
  }
  ...
  static getTrackedAttributes(data, _path=[]) {
    ...
    for ( const actor of game.actors ) {
      if ( actor.isTemplate ) foundry.utils.mergeObject(data, actor.toObject().system);
    }
