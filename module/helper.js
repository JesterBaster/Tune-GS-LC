...
export class EntitySheetHelper {
  ...
  static async createDialog(data={}, options={}) {
    ...
    const templates = collection.filter(a => a.getFlag("angelssword", "isTemplate"));
    const defaultType = this.TYPES.filter(t => t !== CONST.BASE_DOCUMENT_TYPE)[0] ?? CONST.BASE_DOCUMENT_TYPE;
    const types = Object.fromEntries(
      this.TYPES.filter(t => t !== CONST.BASE_DOCUMENT_TYPE).map(t => [t, t])
    );
    ...
    return Dialog.prompt({
      ...
      callback: html => {
        ...
        if ( template ) {
          ...
          delete createData.flags.angelssword.isTemplate;
        }
